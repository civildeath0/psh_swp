...
then
	...
	then
		printf "author file found:\t$(cat author)     ✅ \n"
	else
		printf "author file not found     ❌    \n"
	fi
	/bin/rm -rf hello
fi
...
then
	...
	if [ $leak -gt 0 ]
	then
echo "Leaks: $leak byte(s)   ❌   \033[33mUse: valgrind --leak-check=full ./push_swap <VALUES> to find all!"
	else
	echo "Leaks: 0 bytes   ✅"
	fi
	else
	echo "Leaks: valgrind is unable, check your Makefile or SDL/MLX compilers, or run it as a Daemon.    ⚠️"
	fi
...
if [ $res -gt 1500 ]
then
echo "Test result on $from..$to randoms: $res   0️⃣  ❕  5️⃣  ❗️"
let "sred += $res"
elif [ $res -gt 1300 ]
then
echo "Test result on $from..$to randoms: $res   1️⃣  ❕  5️⃣     ✅"
let "sred += $res"
elif [ $res -gt 1100 ]
then
echo "Test result on $from..$to randoms: $res   2️⃣  ❕  5️⃣     ✅  ✅"
let "sred += $res"
elif [ $res -gt 900 ]
then
echo "Test result on $from..$to randoms: $res   3️⃣  ❕  5️⃣     ✅  ✅  ✅"
let "sred += $res"
elif [ $res -gt 700 ]
then
echo "Test result on $from..$to randoms: $res   4️⃣  ❕  5️⃣     ✅  ✅  ✅  ✅"
let "sred += $res"
elif [ $res -gt 0 ]
then
echo "Test result on $from..$to randoms: $res   5️⃣  ❕  5️⃣     ✅  ✅  ✅  ✅  ✅"
let "sred += $res"
fi
elif [ $dif -eq 500 ]
then
...
if [ $res -gt 12 ]
then
echo "Test result on $from..$to randoms: $res    ❌"
else
echo "Test result on $from..$to randoms: $res    ✅"
fi
else
...
if [ $dif -eq 100 ]
    ...
    if [ $koef -gt 1300 ]
    then
    echo "\nMiddle value: \033[33m$koef   Grade is :  0️⃣     ❗️"
    elif [ $koef -gt 1300 ]
    then
    echo "\nMiddle value: \033[33m$koef   Grade is :  1️⃣     ✅"
    elif [ $koef -gt 1100 ]
    then
    echo "\nMiddle value: \033[33m$koef   Grade is :  2️⃣     ✅  ✅"
    elif [ $koef -gt 900 ]
    then
    echo "\nMiddle value: \033[33m$koef   Grade is :  3️⃣     ✅  ✅  ✅"
    elif [ $koef -gt 700 ]
    then
    echo "\nMiddle value: \033[33m$koef   Grade is :  4️⃣      ✅  ✅  ✅  ✅"
    elif [ $koef -gt 0 ]
    then
    echo "\nMiddle value: \033[33m$koef   Grade is :  5️⃣      ✅  ✅  ✅  ✅  ✅"
    fi
elif [ $dif -eq 500 ]
    then
        if [ $koef -gt 11500 ]
